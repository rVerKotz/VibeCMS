...
import { upsertArticle, deleteArticle } from "../utils/actions";
import Link from "next/link";
import Image from "next/image";

export default function DashboardClient({ articles }: { articles: any[] }) {
  ...
  const handleEdit = (article: any) => {
    ...
    setCurrentImage(article.featured_image);
          ...
          <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sticky top-8 transition-colors">
            <div className="flex items-center justify-between border-b border-zinc-100 p-6 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-lg text-white transition-colors ${editingId ? 'bg-blue-600' : 'bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900'}`}>
                        {editingId ? <PenLine className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
                    </div>
                ...
                {editingId && (
                    <button onClick={resetForm} className="text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300">
                        Batal
                    </button>
                )}
            </div>
            ...
            >
              ...
              <div className="space-y-2">
                 <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Featured Image</label>
                 <div 
                 ...
                 >
                    {previewUrl || currentImage ? (
                        ...
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="text-xs text-zinc-500">Klik untuk upload gambar</p>
                        </div>
                    )}
                    <input 
                        ref={fileInputRef}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    >
                        <option value="draft" className="dark:bg-zinc-900">Draft (Simpan dulu)</option>
                        <option value="published" className="dark:bg-zinc-900">Publish (Tayangkan)</option>
                    </select>
                </div>
              </div>
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition shadow-lg shadow-zinc-900/10 dark:shadow-none ${
                    isSubmitting ? 'bg-zinc-500 cursor-not-allowed' : 'bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200'
                }`}
              >
             ...
             <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900 overflow-hidden transition-colors">
                <div className="flex items-center justify-between border-b border-zinc-100 p-6 dark:border-zinc-800">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-zinc-900 dark:text-zinc-50">Daftar Artikel</h3>
                    </div>
                ...
                {articles.length === 0 ? (
                    <div className="p-12 text-center">
                        ...
                        <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Belum ada artikel</h3>
                        <p className="text-sm text-zinc-500 mt-1 max-w-sm mx-auto">
                            Mulai menulis artikel pertama Anda melalui formulir di samping kiri.
                        </p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-zinc-50 text-zinc-500 dark:bg-zinc-950/50 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-medium text-right w-24">Views</th>
                                    <th className="px-6 py-4 font-medium text-right w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                {articles.map((article) => (
                                    <tr key={article.id} className="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                {article.featured_image && (
                                                    <div className="w-8 h-8 rounded bg-zinc-100 relative overflow-hidden shrink-0">
                                                        <Image src={article.featured_image} alt="" fill className="object-cover" />
                                                    </div>
                                                )}
                                                <div>
                                                    <div className="font-medium text-zinc-900 dark:text-zinc-100 line-clamp-1">{article.title}</div>
                                                    <Link href={`/articles/${article.id}`} target="_blank" className="text-xs text-blue-500 hover:underline flex items-center gap-1 mt-0.5">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <form action={deleteArticle}>
                                                    ...
                                                    <button className="text-zinc-400 hover:text-red-500 transition-colors p-2 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20" title="Hapus Artikel">
